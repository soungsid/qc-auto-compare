"""
Mitsubishi Quebec Spider — scrapes new vehicle inventory from Mitsubishi dealers in Quebec.

Priority models: Mirage, RVR (most affordable Mitsubishi lineup).
"""
...
MITSUBISHI_MODELS = [
    "Mirage", "Mirage G4", "RVR", "Eclipse Cross",
    "Outlander", "Outlander PHEV", "Galant",
]

SELECTORS = {
    "cards": "div.vehicle-card, article.vehicle, li.srp-vehicle, div.inventory-item",
    "title": "h2, h3, .vehicle-title, .car-name",
    "trim": ".trim, .edition, [data-trim]",
    "msrp": ".msrp, .starting-price, [data-msrp]",
    "sale_price": ".price, .sale-price, .discounted-price",
    "drivetrain": "[data-drivetrain], .drivetrain, .awd",
    "vin": "[data-vin]",
    "stock": "[data-stock]",
    "image": "img.vehicle-image, img[src*='mitsubishi']",
    "link": "a[href*='inventaire'], a[href*='vehicle'], a.vehicle-cta",
    "color": ".color, [data-color], .exterior",
    "next_page": "a.next, .pager-next a, a[rel='next']",
}


class MitsubishiQCSpider(BaseDealerSpider):
    """
    Spider for Mitsubishi Quebec dealer websites.

    Usage:
        scrapy crawl mitsubishi_qc_spider
        scrapy crawl mitsubishi_qc_spider -a dealer_slug=mitsubishi-montreal
    """

    name = "mitsubishi_qc_spider"
    brand = "Mitsubishi"
    ...
    def _parse_model(self, title: str) -> Optional[str]:
        ...
        # Sort by length desc so "Mirage G4" matches before "Mirage"
        for model in sorted(MITSUBISHI_MODELS, key=len, reverse=True):
