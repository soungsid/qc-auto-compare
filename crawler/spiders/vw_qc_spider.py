"""
Volkswagen Quebec Spider — scrapes new vehicle inventory from VW dealers in Quebec.
"""
...
VW_MODELS = [
    "Golf", "Golf GTI", "Golf R", "Golf Alltrack", "Jetta", "Jetta GLI",
    "Passat", "Arteon", "ID.4", "ID. Buzz", "ID.Buzz",
    "Tiguan", "Atlas", "Atlas Cross Sport", "Taos",
]


class VwQCSpider(BaseDealerSpider):
    """
    Spider for Volkswagen Quebec dealer websites.

    Usage:
        scrapy crawl vw_qc_spider
        scrapy crawl vw_qc_spider -a dealer_slug=desjardins-volkswagen
    """
    ...
    def _parse_model(self, title: str) -> Optional[str]:
        ...
        # Longer names first to avoid partial matches (e.g. "Golf GTI" before "Golf")
        for model in sorted(VW_MODELS, key=len, reverse=True):
            if model.lower() in title_lower:
                return model
        return None

    def _parse_script_data(self, response: Response, dealer: dict) -> Iterator[dict]:
        yield from self.json_ld_fallback(response, dealer, "Volkswagen", self._parse_model)
