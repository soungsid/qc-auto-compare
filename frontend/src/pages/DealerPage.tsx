...
export function DealerPage({ slug }: Props) {
  ...
  if (dealerLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center text-acier-400">
  ...
  return (
    <div className="min-h-screen bg-creme-200 dark:bg-dark-primary text-charbon-900 dark:text-creme-300 flex flex-col">
      <Navbar />

      {/* Dealer Info */}
      <div className="border-b border-creme-400 dark:border-charbon-800 bg-white dark:bg-charbon-900">
        <div className="mx-auto max-w-screen-xl px-6 py-4">
          <a href="/" className="mb-2 inline-block text-xs text-charbon-700 dark:text-creme-500 hover:underline">
            ← Retour au listing
          </a>
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-2xl font-bold text-charbon-900 dark:text-creme-300">{dealer.name}</h1>
              <p className="text-sm text-acier-500 dark:text-acier-400">
                {dealer.brand} · {dealer.city}
              </p>
            </div>
            <div className="text-right text-sm text-acier-500 dark:text-acier-400">
              {dealer.phone && (
                <p>
                  <a href={`tel:${dealer.phone}`} className="text-charbon-700 dark:text-creme-500 hover:underline">
                    {dealer.phone}
                  </a>
                </p>
              )}
              {dealer.website && (
                <p>
                  <a
                    ...
                    className="text-charbon-700 dark:text-creme-500 hover:underline"
                  >
                    Site officiel ↗
                  </a>
                </p>
              ...
              {dealer.last_crawled_at && (
                <p className="mt-1 text-xs text-acier-400 dark:text-acier-500">
                  Dernier crawl :{' '}
